...
Now we can build emulators for these $q$ coefficients. We can do this simultaneously for all $q$ (as long as we're happy using the same assumptions for each):

```{r, eval = FALSE}
em_coeffs <- BasisEmulators(tDataC, q, mean_fn = 'step', maxdf = 5)
```
...
ValidateGasp(em_coeffs[[1]]);ValidateGasp(em_coeffs[[2]]);ValidateGasp(em_coeffs[[3]]);ValidateGasp(em_coeffs[[4]])
