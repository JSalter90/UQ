...
#' 
#' Other cases (e.g., emulating l outputs independently, but wishing to calculate the l-dimensional implausibility rather than l independent 1D implausibilities) are not supported by this function.
#' 
...
#'
#' @examples
ImplFieldTrue <- function(TrueOutput, Obs, ObsVar, DiscVar = 0){
  ...
  Impl <- rep(NA, n)
  for (i in 1:n){
    Impl[i] <- t(Obs - TrueOutput[,i]) %*% tmp_inverse %*% c(Obs - TrueOutput[,i])
  }
