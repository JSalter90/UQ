...
Predictions work the same for each type of emulator:
...
We can calculate the implausibility for an individual output, given a set of predictions:

```{r}
impl_t150 <- Impl(preds_t150, Obs = obs[150], ObsVar = 0.5, kmax = 1)
summary(impl_t150)
summary(impl_t150$Impl)
impl_t150$NROY
impl_t150$Pass_kth
impl_t150$Pass_N
```

Or for multiple outputs (independently). If `obs_var` is only given as a single value, the same variance is assumed for each output, but could instead be given as a vector of length 4:

```{r}
impl_time <- Impl(preds_time, Obs = obs[c(50,150,300,500)], ObsVar = 0.5, kmax = 1)
summary(impl_time)
summary(impl_time$Impl)
impl_time$NROY
impl_time$Pass_kth
impl_time$Pass_N
```

Note that the 2nd column in the latter is the same as the implausibility from `impl_t150`.

It will usually be more convenient to combine prediction and calibration:
...
This returns both the predictions (identical to what we constructed by hand):
...
`hm$Design` contains a column classifying each point as NROY or not:

```{r}
head(hm_time$Design)
```

Taking the output, can visualise the NROY space in 1D and 2D, with the truth overlaid:

```{r, fig.height = 5}
PlotNROY(hm_time$Design, k = 1:4, Truth = truth)
```

For more on HM, and functionality for performing multiple waves smoothly, see XXXX.
